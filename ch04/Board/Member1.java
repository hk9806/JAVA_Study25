...
public class Member1 {// 회원에 대한 CRUD
	...
	public void modify(Scanner input, Member1[] members) {
		...
		Member1 modifymember = new Member1();
		modifymember.id = id;
		modifymember.pw = pw;

		for (int i = 0; i < members.length; i++) {
			if (members[i] != null && members[i].id.equals(modifymember.id) && members[i].pw.equals(modifymember.pw)) {
	...
	public void delete(Scanner input, Member1[] members) {
		System.out.println("회원 탈퇴를 시작합니다.");

		System.out.println("삭제할 id를 입력하세요");
		String id = input.next();

		Member1 deletemember = new Member1();
		deletemember.id = id;

		for (int i = 0; i < members.length; i++) {
			if (members[i] != null && members[i].id.equals(deletemember.id)) {
