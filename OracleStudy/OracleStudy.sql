...
--listagg 사용 부서별 사원이름 가로 정렬 출력

SELECT DEPTNO, JOB, MAX(SAL) FROM EMP GROUP BY DEPTNO, JOB ORDER BY DEPTNO, JOB;
--부서,직책별 그룹화하여 최고 급여 데이터 출력

--PRIVOT, UNPRIVOT
--PRIVOT:기존 테이블 행을 열로 바꿈
--UNPRIVOT:기존 테이블 열을 행으로 바꿈
SELECT*FROM (SELECT DEPTNO, JOB, SAL FROM EMP) PIVOT(MAX(SAL)FOR DEPTNO IN (10,20,30)) ORDER BY JOB;
--PIVOT 이용 직책, 부서별 최고급여 2차원 표 형태로 출력
SELECT*FROM (SELECT JOB, DEPTNO, SAL FROM EMP) PIVOT(MAX(SAL) FOR JOB IN('CLERK' AS CLERK,
'SALESMAN' AS SALESMAN,'PRESIDENT' AS PRESIDENT,'MANAGER' AS MANAGER,'ANALYST' AS ANALYST))
ORDER BY DEPTNO;
--PIVOT 이용 부서,직책별 최고급여 2차원 표 형태로 출력
...
--JOIN:두 개 이상의 테이블을 연결하여 하나의 테이블 처럼 출력하는 방식
--등가조인(EQUI JOIN):테이블 연결 후 출력행을 각 테이블의 특정 열에 일치한 데이터를 기준으로 선정
--자체조인(SELF JOIN):하나의 테이블을 여러 테이블처럼 활용
--외부조인(OUTER JOIN):두 테이블 간 조인 수행에서 조인 기준 열의 한쪽이 NULL이어도 강제로 출력
--내부조인(INNER JOIN0; 조인 조건에 해당하는 데이터가 존재할 경우에만 출력(등가,자체조인 해당)
--NTURAL JOIN:대상이 되는 두 테이블에 이름과 자료형이 일치하는 열을 찾아 그 열을 기준으로 등가조인 해주는 방식
...
SELECT E1.EMPNO, E1.ENAME, E1.MGR, E2.EMPNO AS MGR_EMPNO, E2.ENAME AS MER_ENAME
...
--JOIN USING: NATURAL JOIN은 자동으로 기준 열을 지정하나 USING키워드를 사용하면 키워드에 기준을 명시
SELECT E.EMPNO, E.ENAME, E.JOB, E.MGR, E.HIREDATE, E.SAL, E.COMM,
DEPTNO, D.DNAME, D.LOC FROM EMP E JOIN DEPT D USING(DEPTNO) WHERE SAL>=3000 ORDER BY DEPTNO, E.EMPNO;

--JOIN ON:조인 기준 조건식은 ON에 명시하고 그밖의 출력행을 걸러내기 위해 WHERE조건식을 따로 사용
SELECT E.EMPNO, E.ENAME, E.JOB, E.MGR, E.HIREDATE, E.SAL, E.COMM, E.DEPTNO, D.DNAME, D.LOC
FROM EMP E JOIN DEPT D ON(E.DEPTNO = D.DEPTNO) WHERE SAL<=3000 ORDER BY E.DEPTNO, EMPNO;

--OUTER JOIN:키워드로 외부조인에 사용, FROM절에서 외부 조인 선언
SELECT E1.EMPNO, E1.ENAME, E1.MGR, E2.EMPNO AS MGR_EMPNO, E2.ENAME AS MGR_ENAME
FROM EMP E1 LEFT OUTER JOIN EMP E2 ON (E1.MGR=E2.EMPNO) ORDER BY E1.EMPNO;

SELECT E1.EMPNO, E1.ENAME, E1.MGR, E2.EMPNO AS MGR_EMPNO, E2.ENAME AS MGGR_ENAME 
FROM EMP E1 RIGHT OUTER JOIN EMP E2 ON (E1.MGR=E2.EMPNO) ORDER BY E1.EMPNO, MGR_EMPNO;
